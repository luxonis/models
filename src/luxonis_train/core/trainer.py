...
class Trainer:
    def __init__(self, cfg: Union[str, dict], args: Optional[dict] = None):
        """Main API which is used to create the model, setup pytorch lightning
        environment and perform training based on provided arguments and config.

        Args:
            cfg (Union[str, dict]): path to the config file or used to setup training
            args (Optional[dict]): argument dict provided through command line,
        ...
        self.lightning_module = LuxonisModel.from_config(self.run_save_dir, self.cfg)
    ...
    def train(self, new_thread: bool = False):
        ...
        with LuxonisDataset(
            self.cfg.get("dataset.dataset_name"),
        ) as dataset:
            ...
            if self.cfg.get("train.use_weighted_sampler"):
                ...
                if len(classes_count) == 0:
                    warnings.warn(
                        "WeightedRandomSampler only available for classification tasks."
                        " Using default sampler instead."
                    )
                else:
            ...
            if self.val_augmentations is None:
