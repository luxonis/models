...
class MeanAveragePrecision(detection.MeanAveragePrecision):
    def __init__(self, **kwargs):
        """Wrapper for torchmetrics.detection.MeanAveragePrecision that omits some
...
class MeanAveragePrecisionKeypoints(Metric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            num_keypoints (int): Number of keypoints
            kpt_sigmas (Optional[Tensor], optional): Sigma for each keypoint to weigh
            its importance,
                if None use same weights for all. Defaults to None.
        ...
        containing key-values for a single image.
            Parameters that should be provided per dict:
            - boxes (torch.FloatTensor): Tensor of shape ``(num_boxes, 4)``
            containing ``num_boxes`` detection
                boxes of the format specified in the constructor. By default, this
                method expects ``(xmin, ymin, xmax, ymax)``
                in absolute image coordinates.
            - scores (torch.FloatTensor): Tensor of shape ``(num_boxes)``
            containing detection scores for the boxes.
            - labels (torch.IntTensor): Tensor of shape ``(num_boxes)``
        ...
        containing key-values for a single image.
            Parameters that should be provided per dict:
            - boxes (torch.FloatTensor): Tensor of shape ``(num_boxes, 4)`` containing
            ``num_boxes`` ground truth
                boxes of the format specified in the constructor. By default, this
                method expects ``(xmin, ymin, xmax, ymax)``
                in absolute image coordinates.
            - labels: :class:`~torch.IntTensor` of shape ``(num_boxes)`` containing
            0-indexed ground truth classes for the boxes.
            - iscrow (torch.IntTensor): Tensor of shape ``(num_boxes)`` containing 0/1
            values indicating whether
                the bounding box/masks indicate a crowd of objects. Value is optional,
                and if not provided it will
                automatically be set to 0.
            - area (torch.FloatTensor): Tensor of shape ``(num_boxes)`` containing the
            area of the object. Value if
                optional, and if not provided will be automatically calculated based on
                the bounding box/masks provided.
                Only affects which samples contribute to the `map_small`, `map_medium`,
                `map_large` values
            - keypoints (torch.FloatTensor): Tensor of shape (N, 3*K) and in format
            [x,y,vis,x,y,vis,...] where `x` an `y`
                are unnormalized keypoint coordinates and `vis` is keypoint visibility.
