...
class LinearBottleneck(nn.Module):
    def __init__(self, in_channels, channels, t, kernel_size=3, stride=1, **kwargs):
        ...
        if t != 1:
            ...
            out.append(
                ConvModule(in_channels=in_channels, out_channels=dw_channels, kernel_size=1, activation=nn.ReLU6(inplace=True))
            )
        else:
            dw_channels = in_channels
        out.append(
            ConvModule(in_channels=dw_channels, out_channels=dw_channels*1, kernel_size=kernel_size, stride=stride,
                padding=(kernel_size//2), groups=dw_channels, activation=nn.ReLU6(inplace=True)
            )
        )
        out.append(
            ConvModule(in_channels=dw_channels, out_channels=channels, kernel_size=1, activation=nn.Identity())
        )
...
class ReXNetV1_lite(nn.Module):
    def __init__(self, fix_head_stem: bool = False, divisible_value: int = 8, input_ch: int = 16,
        ...
        first_channel = make_divisible(int(round(first_channel * multiplier)), divisible_value)

        in_channels_group = []
        channels_group = []

        features.append(
            ConvModule(3, first_channel, kernel_size=3, stride=2, padding=1, activation=nn.ReLU6(inplace=True))
        )

        for i in range(self.num_convblocks):
            inplanes_divisible = make_divisible(int(round(inplanes * multiplier)), divisible_value)
            if i == 0:
        ...
        features.append(
            ConvModule(in_channels=c, out_channels=pen_channels, kernel_size=1, activation=nn.ReLU6(inplace=True))
        )
