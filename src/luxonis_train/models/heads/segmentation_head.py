...
class SegmentationHead(BaseSegmentationHead):
    def __init__(self, n_classes: int, prev_out_shapes: list, original_in_shape: list, attach_index: int = -1, **kwargs):
        ...
        super().__init__(n_classes=n_classes, prev_out_shapes=prev_out_shapes, original_in_shape=original_in_shape,
        ...
        if not num_up.is_integer():
            warnings.warn("Segmentation head's output shape not same as original input shape.")
            num_up = round(num_up)
