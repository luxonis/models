...
from luxonis_train.models.modules import BlockRepeater, ConvModule, RepVGGBlock
from luxonis_train.utils.general import make_divisible
from luxonis_train.utils.types import ModulePacket


class RepPANNeck(LuxonisModule):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        """RepPANNeck from `YOLOv6: A Single-Stage Object Detection Framework
        for Industrial Applications`,
        https://arxiv.org/pdf/2209.02976.pdf. It has the balance of feature
        fusion ability and hardware efficiency.
        ...
        Args:
            ...
            attach_index (int, optional): Index of previous output that the head
            attaches to. Defaults to -1.
        ...
        in_channels_next = self.input_shapes[self.attach_index - 1][1]
        ...
        for i in range(1, num_heads):
            ...
            in_channels_next = self.input_shapes[self.attach_index - (i + 1)][1]
    ...
    def forward(self, inputs: list[ModulePacket]) -> ModulePacket:
        x0 = inputs[0]["features"][self.attach_index]
        up_block_outs = []
        for i, up_block in enumerate(self.up_blocks):
            conv_out, x0 = up_block(
                x0, inputs[0]["features"][self.attach_index - (i + 1)]
            )
    ...
    def validate(self, inputs: list[Tensor]) -> None:
        ...
        if attach_index < 0:
            attach_index = abs(attach_index) - 1
        assert len(inputs) > attach_index

    def _validate_num_heads_and_attach_index(self, num_heads: int):
        """Checks if specified number of heads is supported and if cumulative
        offset is valid"""
        ...
        if len(self.input_shapes) - (abs(self.attach_index) - 1 + num_heads) < 0:
            raise ValueError(
                "Cumulative offset (abs(attach_index)+num_head) out of range."
            )

    def _fit_to_num_heads(self, channels_list: list, num_repeats: list):
        """Fits channels_list and num_repeats to num_heads by removing or
        adding items. Also scales the numbers based on offset"""
        if self.num_heads == 3:
            pass
