...
class YoloV7PoseLoss(nn.Module):
    # Compute losses
    def __init__(self, n_classes, cls_pw=1.0, obj_pw=1.0, gamma=2, alpha=0.25, label_smoothing=0.0,
        box_weight=0.05, kpt_weight=0.10, kptv_weight=0.6, cls_weight=0.6, obj_weight=0.7, 
        anchor_t=4.0, **kwargs):
        super().__init__()
        ...
        self.balance = {
            3: [4.0, 1.0, 0.4]
        }.get(head_attributes.get("nl"), [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
    ...
    def __call__(self, kpt_pred, kpt, **kwargs):
        ...
        for i, pi in enumerate(kpt_pred):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox.T, tbox[i].to(device), box_format="xywh", iou_type="ciou")
