...
class Registry:
    ...
    def register_module(
        ...
        Args:
            name (Optional[str], optional): Name of the module, if None then use class
            name. Defaults to None.
            module (Optional[type], optional): Module class to be registered.
            Defaults to None.
            force (bool, optional): Wheather to override an existing class with the
            same name. Defaults to False.

        Returns:
            Union[type, Callable[[type], type]]: Module class or register
            function if used as a decorator
        """
    ...
    def _register_module(
        ...
        Args:
            ...
            module_name (Optional[str], optional): Name of the module, if None use
            class name. Defaults to None.
            force (bool, optional): Weather to override an existing class with the
            same name. Defaults to False.
        ...
        if register:
            if not hasattr(new_class, "_REGISTRY"):
                raise AttributeError(
                    f"Class `{name}` does not have a `_REGISTRY` attribute."
                )
            new_class._REGISTRY.register_module(  # type: ignore
                name=register_name or name, module=new_class
            )
        return new_class


MODULES = Registry(name="modules")
